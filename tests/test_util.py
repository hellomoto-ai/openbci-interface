...
$$$'''
    ...
    class SerialMock:
        ...
        def write(self, val):
            if val == b'v':
                self.buffer = TestListDevices.firmware_strings[self.port]
            else:
                raise ValueError(
    ...
    def test_list_devices(self, mocker, filter_pattern, expected):
        mocker.patch.object(
            util.serial, 'Serial', self.SerialMock)
        mocker.patch.object(
            util.serial.tools.list_ports, 'comports', self.comports_mock)
